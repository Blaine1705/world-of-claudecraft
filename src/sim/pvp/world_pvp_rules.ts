// World PvP (the /pvp flag): the pure rules. Who is hostile to whom once both
// sides opted in, what a kill stakes, and how the stake and the honor pool are
// split among everyone who worked for it. No SimContext, no rng, no clock: every
// function here is a plain function of its arguments so the sim (isHostileTo,
// the death hook), the renderer (nameplate colour), and the HUD (target frame,
// the auto-attack gate, the World PvP tab) can all read the same verdict and
// never drift. The ctx-bound system that owns the flag state, the assist books,
// the 5-minute disarm clock, and the kill resolution is world_pvp.ts.
 ...
 *  their purse, whichever is the minimum"). In copper: 5 gold. */
 ...
 * isOwnedPetHostile). Both must carry the flag, and two players who share a
 * party or raid, or a guild, are never hostile: the group is the "faction" of
 * a factionless world, and a flagged mage's Blizzard must never land on the
 * healer standing beside them. Symmetric by construction.
 */
export function worldPvpPairHostile(a: Entity, b: Entity, inSameParty: boolean): boolean {
