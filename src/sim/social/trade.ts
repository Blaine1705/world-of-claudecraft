...
// The per-copy exclusion for a SOULBOUND def traded to `counterpartyName` at
// `nowMs`: the bound lock above, plus the bind-on-pickup party trade window
// (bop_trade_window.ts). Only a copy whose window is unexpired AND names the
// counterparty may cross; a plain soulbound stack (instance undefined) never
// qualifies, which keeps the historical everything-soulbound-is-refused
...
function soulboundOfferSkip(
  ...
  return (instance) =>
    isTradeLocked(instance) || !partyTradeWindowAllows(instance, counterpartyName, nowMs);
}
...
export function tradeSetOffer(
  ...
  for (const [itemId, count] of merged) {
    ...
    // AND names the counterparty may be offered, clamped silently (the same
    // silent posture the old unconditional soulbound drop had). The
    // windowed-copy pre-gate runs BEFORE the counterparty and clock resolve,
    // so a decoupled ctx with no inventory never touches those members.
    if (ITEMS[itemId]?.soulbound) {
      ...
      const counterpartyName = ctx.players.get(otherPid)?.name;
      if (counterpartyName === undefined) continue;
      const skip = soulboundOfferSkip(counterpartyName, ctx.lockoutNowMs());
...
function shippedOfferUnits(
  ...
  // soulbound line never touches the players map or the lockout clock, which
  // decoupled test ctxs may not model). `null` records an unresolvable
  ...
  const skipFor = (itemId: string): ((instance: ItemInstancePayload | undefined) => boolean) => {
    ...
    if (soulboundSkip === undefined) {
      const name = ctx.players.get(toPid)?.name;
      soulboundSkip = name === undefined ? null : soulboundOfferSkip(name, ctx.lockoutNowMs());
    }
...
export function tradeConfirm(ctx: SimContext, pid?: number): void {
  ...
  const valid =
    ...
    offerCovered(ctx, session.offerA.items, session.a, session.b) &&
    offerCovered(ctx, session.offerB.items, session.b, session.a);
  if (!valid) {
    for (const tPid of [session.a, session.b])
  ...
  const fitsAfterSwap = (
    ...
    const shipped = shippedOfferUnits(ctx, receives, giver.entityId, meta.entityId, scratchGiver);
  ...
  const grantsToB = removeOffer(ctx, session.offerA.items, session.a, session.b);
  const grantsToA = removeOffer(ctx, session.offerB.items, session.b, session.a);
...
// instead of shipping.
function offerCovered(ctx: SimContext, items: InvSlot[], pid: number, otherPid: number): boolean {
  ...
  for (const [itemId, count] of totals) {
    if (ITEMS[itemId]?.soulbound) {
      ...
      const counterpartyName = ctx.players.get(otherPid)?.name;
      if (counterpartyName === undefined) return false;
      const skip = soulboundOfferSkip(counterpartyName, ctx.lockoutNowMs());
