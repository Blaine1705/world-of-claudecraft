...
export function harvestCorpse(
  ...
  // before the claim is spent. `wanted` is empty exactly when no effective
  // component maps to an item, and a corpse carrying at least one mapped tag
  // is one a different pick would have paid out on, so the two conditions
  // together say "this selection throws the whole corpse away". Measured
  // pre-fix on old_greyjaw (hide, fang, claw) with ['claw']: the claim was
  // spent, one tier roll was drawn, nothing was granted, and the harvestResult
  // ledger was skipped (it is gated on `granted.length > 0`), so the player
  // burned a single-use corpse for no items and NO chat line at all. Nine
  // shipped templates mix mapped and unmapped families, and on the three
  // `gills, hide` murlocs a single checkbox is enough to hit it.
  //
  // Placed here, beside the capacity gate, for the same three reasons that one
  ...
  // Mutually exclusive with the bags-full gate below, which needs
  // `wanted.length > 0`, so neither can mask the other's message.
  //
  // Deliberately NOT narrowed inside effectiveFocusComponents (professions/
  // gathering.ts) the way an uncarried tag is (#2504): the concentration bonus
  // is `taggedComponents.length - effectiveChosen.length`, so dropping a
  ...
  // bonus-2 ['hide'] world (rough_hide 4 + a pristine_hide), and the
  ...
  // tests/corpse_harvest_sim.test.ts and tests/corpse_harvest_result_event.ts
  // both pin. That is what the second condition buys, and it is why the pin
  // stays green rather than being re-argued away.
  //
  ...
  if (wanted.length === 0 && (componentTags ?? []).some((tag) => HARVEST_COMPONENT_ITEMS[tag])) {
