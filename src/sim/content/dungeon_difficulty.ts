...
export interface HeroicDungeonTuning {
  ...
  // Optional per-mob override for encounter mechanics whose tuning must be
  // decoupled from melee after the level-22 transform.
...
export const HEROIC_DUNGEON_TUNING: Record<string, HeroicDungeonTuning> = {
  ...
  // claim possible while Normal continues to use the base template. Parse
  // calibration keeps the boss at 210k so Normal remains accessible without
  // making Heroic a direct carry-over for a geared Nythraxis raid.
