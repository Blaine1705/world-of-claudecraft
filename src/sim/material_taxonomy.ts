...
// source table (guarded by tests/crafting_materials_quality.test.ts), and the
// unclassified non-poor trophies/keepsakes are in no source table either (the
// completeness tripwire in tests/material_taxonomy.test.ts enumerates them, so
// a future junk item must be classified explicitly).
//
...
//
// HARD RULE: no `src/sim/**` file may import this module (enforced by a scan
...
// because nothing here sits inside data.ts's evaluation cycle. A sim-side
// importer would pull the derive into that cycle, and what happens then
// depends on which module the host reaches first: one entry order crashes at
// load (the source tables are still undefined when the derive runs), the
// other works perfectly, so the defect is invisible under one entry point and
// fatal under another. That order-dependence is exactly what a static import
// scan catches and no runtime test reliably can.
...
function deriveMaterialItemIds(): ReadonlySet<string> {
  ...
  for (const row of Object.values(MATERIAL_GRADES)) sources.add(row.fineItemId);
  // Corpse-harvest components and their pristine-specimen jackpots.
  for (const id of Object.values(HARVEST_COMPONENT_ITEMS)) sources.add(id);
  for (const id of Object.values(HARVEST_COMPONENT_SPECIMENS)) sources.add(id);
  ...
  for (const id of Object.values(SALVAGE_MATERIAL_BY_QUALITY)) sources.add(id);
  ...
  return new Set([...sources].filter((id) => ITEMS[id]?.kind === 'junk'));
}
