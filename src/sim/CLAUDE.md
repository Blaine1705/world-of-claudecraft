...
| `dead_gate.ts` | `refusedWhileDead`: the shared while-dead refusal for command families whose result event would otherwise emit unconditionally (the profession-action wrappers on `Sim`, mobile-station placement, the rift forge); emits the matcher-covered error line and suppresses the result event, draws NO rng |
