...
| `mob/immobile_evade.ts` | `immobileEvadeSnapsHome`: the evade walk-home for an authored-immobile mob (moveSpeed 0, the Broodmother egg clutch) is a snap onto its spawn point, since a zero step never arrives; consumed by the `locomotion.ts` evade arm before the walk; pure, no rng; pinned by `tests/immobile_mob_evade.test.ts` |
...
| `loot/kill_participation.ts` | kill-time participation: where a member (a released ghost's corpse included) is judged from, the claim-wide share inside an instance, and the leave-time tap re-anchor |
