...
| `transport_ferry.ts` | the scheduled Eastbrook ferry (Phase 2 of the ferry): run at the top of the tick prologue (after world bosses, before ground AoEs; timed under the next `lap` mark), it applies the berth deck gates (`transport_gates.ts` over the pure `transport_schedule.ts` timetable), boards everyone on deck at a departure (a fighter, a corpse or a spirit is set down on the pier; a pet is parked in the delve pet stash), carries passengers with the ship's pose (`Sim.updatePlayerMovement` skips them), releases them docked at the far berth, and maps a save taken aboard to a pier (`ferrySavePosition`). `Sim` also syncs the gates at construction. The dev skip is `ctx.transportClockOffset` (`dev/ferry_dev.ts`); draws no rng |
