...
| `reliquary.ts` | The Reliquary catalog state: `noteRelicItemFind`/`noteReliquaryMark` (first-find provenance + the capped recent ring), `noteRelicObtain` (the per-relic obtain tally the grant hub bumps for WORLD-SOURCED acquisitions only, skipped for every `movement: true` grant: trade, mail, market, an enchant re-mint, an unbind peel, a returned commission), the sparse blob round-trip (`serializeReliquaryState` plus THREE exported load entry points, not one: `restoreReliquaryState` and the two narrow helpers `restoreReliquaryMarks`/`restoreReliquaryRecent` that `server/character_sheet.ts` calls directly so a public sheet read need not pay a full restore for two surfaces; `restoreReliquaryState` delegates to those same two helpers, so the catalog filters and the corrupt-value tolerance have ONE implementation and cannot drift, and its delegated arms merge into the fresh state in place like every sibling arm; the tally rides folded onto its `firstFind` entry rather than a fourth top-level key), the wire memo (`reliquaryWireJson` plus the `reliquaryWireCacheProbe` test seam, two identity-keyed `WeakMap`s so a staggered heavy refresh with nothing changed reuses the built string), the catalog-index memo (`catalogIndexFor` plus the `reliquaryCatalogIndexProbe` test seam, a `WeakMap` keyed on the PAGE TABLE so the de-duped per-kind id lists behind every `total` are walked once per table and a caller's own page array can never answer with the default catalog's index), the scoring-pages memo (`completionScoringPages` plus the `reliquaryScoringPagesProbe` test seam, the same identity-keyed `WeakMap` regime one layer above it: it drops every `excludeFromCompletion` page from BOTH sides of every completion pair and is IDENTITY-STABLE, answering an unflagged table with the caller's own array, which is what keeps the catalog-index memo keys hot instead of rebuilding per read), the pure completion/rank readouts (`pageCompletion`, `catalogCharacterCompletion`, `curatorRankFromOwned`, `clearCountForSource` over live `deedStats`/`delveClears`), and the illumination emit (module-private `emitReliquaryUnlock`); cosmetic only (titles/borders ride `deeds.ts` grants, and the tally is information that scores nothing), draws NO rng |
