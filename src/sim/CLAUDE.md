...
- `material_ids.ts` + `material_derivation.ts`: the ONE material-set derive both sides
  ...
  table arrives as a parameter), which lets the eager UI-side taxonomy and the sim's
  lazy `materialItemIds()` memo in `material_ids.ts` keep separate evaluation timing;
  `vaultMaterialIds()` delegates to the memo.
