...
| `world_quests.ts` + `world_quest_state.ts` + `world_quest_combat.ts` | overworld area auto-start, kill credit, realm-reset rollover and automatic rewards; state adapters own fresh/load/save normalization and rotation claims; combat expeditions own session-only enemies, wave/hunt/defense progression and teardown behind SimContext |
