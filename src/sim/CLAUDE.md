...
| `pet/pet_match_return.ts` | the arena-shaped-match pet round trip (`snapshotMatchPet` at match formation, `restoreMatchPet` at the end of `returnFromArena`): a pet that walks in alive walks back out alive, at the hp it carried, matched by ENTITY ID so a pet parted with mid-bout is never handed back. The pools half of the same doctrine (issue #1600) is `ArenaReturnPools`; draws NO rng |
