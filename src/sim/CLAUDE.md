...
| `dev_commands.ts` + `dev/` | the `ctx.devCommands` gated `/dev` cheat surface: `handleDevChat` (re-exported by `social/chat.ts` for the chat router), `spawnMobsForDev`/`despawnMobsForDev` (dev-spawned mobs are torn down in `removePlayer`), `resetCombatForDev`; pinned by `tests/dev_commands.test.ts`. `dev/` holds the dev-only playtest harnesses, never reachable in production: `bis_gear.ts` (`/dev bis`: applies the selected spec's frozen top-parse loadout from `parse_bis_loadouts.ts`, with the deterministic epic scorer only as the spec-less fallback, draws no rng) and `cascade_playtest.ts` (ALLOW_DEV_COMMANDS-gated manual-playtest metrics: pure observation, never feeds a gameplay decision or the wire) |
