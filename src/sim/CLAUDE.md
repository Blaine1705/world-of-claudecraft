...
| `weekly_rewards.ts` + `weekly_reward_open.ts` + `character_storage.ts` | Weekly Vault activity credits, bounded earned slots, roll-on-open preparation and durable acknowledgment, class-filtered loot pools and guarded claims; character storage load/save boundary. Host reset calendar lives in `src/reset_calendar.ts` |
