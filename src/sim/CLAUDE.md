...
| `items.ts` + `auto_equip.ts` | equip/use/discard + vendor buy/sell/buyback command bodies; exact-copy automatic equipment selection after item grants (no aimed slot: the hand routes by spec exactly as an explicit equip does) |
...
| `unstuck.ts` | `/unstuck` recovery countdown, the graveyard move (alive) or graveyard revive (dead), cancellation, and cooldown. Never a death; charges Unstuck Sickness only on a repeat inside the hour window (the first use in an hour is free). The hidden system cooldowns behind it, the anti-relog retry timer and the sickness window marker (`unstuckOwesSickness`/`markUnstuckCompleted`), live in the `unstuck_cooldown.ts` pure leaf so competitive resets and the `/cooldowns` readout can share them without a cycle |
...
| `transport_ferry.ts` | the scheduled Eastbrook ferry (Phases 2 and 3): run at the top of the tick prologue (after world bosses, before ground AoEs; timed under the next `lap` mark), it applies the berth deck gates (`transport_gates.ts` over the pure `transport_schedule.ts` timetable and its sea lanes), and whenever the ship's pose changes carries every player aboard it rigidly to the same deck spot (`transport_deck.ts` `carryWithDeck`; a corpse or a spirit on deck at a departure is set down on the pier, and so is anyone on the stowed gangplank; a pet is parked in the delve pet stash). Under way the deck is a kinematic platform the movement kernel stands them on (`ferryDeckPlatform`, bound as `PlayerMotionDeps.platform`; the pure platform queries are `physics/platform.ts`), so passengers walk, jump and climb it; `Sim.updatePlayerMovement` hands a passenger to `stepPassenger` (a feared one cowers, the forced movement modes are dropped). Mooring ends the voyage where they stand and marks the ferry deed; a save taken aboard maps to a pier (`ferrySavePosition`). `Sim` also syncs the gates at construction. The dev skip is `ctx.transportClockOffset` (`dev/ferry_dev.ts`, which also carries passengers across a jump and boards a ship under way); holds no state (the carry reads the pose one tick back); draws no rng |
...
| `loot/enemy_quality.ts` | `isEligibleEnemyQualitySource` (wild, unowned, non-affix, non-dummy kills) + `rollEnemyLootQuality`: the ONE quality roller, applied once AFTER a source has finished selecting its ordinary drops (corpse loot, world boss, Rift clear rewards), one tier draw per copy with a per-copy stack split; never from pickup/award |
