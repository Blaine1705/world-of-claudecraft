...
// The forge-lift antechamber's inner gate: sealed while the lift "descends"
// (a fixed ride after the instance claim; the room never moves), then swapped
// open in place. Same-room gate, so it is its own template pair rather than
// the room-crossing 'dungeon_door' machinery.
export const IGNIVAR_LIFT_GATE_LOCKED_TEMPLATE = 'ignivar_lift_gate_locked';
export const IGNIVAR_LIFT_GATE_OPEN_TEMPLATE = 'ignivar_lift_gate_open';
