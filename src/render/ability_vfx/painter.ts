...
// One FULL point-anchored sequence per (caster, ability, quarter-yard cell)
// inside this window: recurring emits at the same aimed point (Frozen Orb's
// flight pulses, a re-triggered trap) degrade to the cheap zone re-hit, while
// simultaneous landings at distinct points each retain their authored impact.
...
export class AbilityVfx {
  ...
  // last full point-sequence time per caster, ability, and quantized point
  // (see POINT_SEQ_REFRACTORY_SEC); stale entries are pruned in place
  ...
  handleSpellfxAt(ev: AbilityVfxSpellfxAtEvent): boolean {
    ...
    const full = abilityVfxFullSpecFor(ev.ability);
    // Recurring emits at the same aimed point replay only the cheap re-hit.
    // Spatially distinct simultaneous impacts (such as Ignivar's three fire
    // cones) each receive the full authored sequence.
