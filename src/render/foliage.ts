...
function buildTrees(
  ...
  if (session) {
    ...
    pineSpec.impostorRows = pineSpec.sets.map((parts, i) =>
      session.registerArchetype('tree', MODEL_URLS.pine[i], parts),
    );
    oakSpec.impostorRows = oakSpec.sets.map((parts, i) =>
      session.registerArchetype('tree', MODEL_URLS.oak[i], parts),
    );
...
function buildDressing(
  ...
  const dressRows = session
    ? {
        bush: session.registerArchetype('dress', MODEL_URLS.bush[0], kindParts.bush),
