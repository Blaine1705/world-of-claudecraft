...
export function syncIgnivarBrandTelegraph(
  ...
  const pulse = danger ? 0.5 + Math.sin(elapsed * 11) * 0.5 : 0;
  ...
  const overheadRing = root.getObjectByName(IGNIVAR_BRAND_OVERHEAD_RING_NAME) as
    | THREE.Mesh
    | undefined;
  const timePulse = 0.5 + Math.sin(elapsed * 5.2) * 0.5;
  const inwardPulse = 0.5 + Math.sin(elapsed * 6.5) * 0.5;
  if (fill)
  ...
  if (runes) {
    ...
    runes.rotation.y = -elapsed * (0.18 + danger * 0.12);
  }
  if (spikes) {
  ...
  if (overheadRing) {
    ...
    overheadRing.rotation.z = elapsed * (0.8 + danger * 0.45);
  }
  if (embers) {
    embers.rotation.y = elapsed * (danger ? 2.6 : 1.45);
