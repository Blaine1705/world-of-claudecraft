...
export function syncIgnivarSoakTelegraph(
  ...
  if (flame) {
    ...
    flame.scale.y = 0.92 + Math.sin(elapsed * 7.5) * 0.1;
  }
  if (beacon) {
    beacon.visible = !readyState;
    beacon.rotation.y = elapsed * 1.35;
    beacon.scale.y = 0.9 + Math.sin(elapsed * 7.5) * 0.08;
    ...
    if (outer) (outer.material as THREE.Material).opacity = 0.18 + Math.sin(elapsed * 6) * 0.04;
    if (core) (core.material as THREE.Material).opacity = 0.54 + Math.sin(elapsed * 8) * 0.1;
    if (embers) embers.rotation.y = elapsed * 1.7;
  }
  if (ready) {
    ready.visible = readyState;
    ready.scale.setScalar(0.96 + Math.sin(elapsed * 5) * 0.04);
  }
  if (swirl) {
    swirl.rotation.y = -elapsed * 0.46;
    (swirl.material as THREE.Material).opacity = readyState ? 0.28 : 0.44;
  }
  if (arrows) {
    const callStrength = readyState ? 0.52 : 0.82 + Math.sin(elapsed * 6) * 0.1;
    (arrows.material as THREE.Material).opacity = callStrength;
    arrows.scale.setScalar(0.98 + Math.sin(elapsed * 6) * 0.015);
  }
}
