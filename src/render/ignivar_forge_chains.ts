...
export function syncIgnivarForgeChainVisual(
  ...
  const pulse = 0.88 + Math.sin(animationTime * (strained ? 22 : 11) + entity.id) * 0.12;
  ...
  for (const child of root.children) {
    ...
    if (child.visible) {
      ...
      child.position.set(0, Math.sin(animationTime * 8 + linkIndex * 1.7) * 0.055, length * along);
      child.scale.setScalar(0.88 + pulse * 0.14);
      child.rotation.z = animationTime * (linkIndex % 2 === 0 ? 0.7 : -0.7);
    }
    linkIndex++;
  }
  ...
  for (const child of root.children) {
    ...
    const flicker = 0.72 + Math.sin(animationTime * 13 + flameIndex * 2.1) * 0.2;
    child.position.set(
      Math.sin(animationTime * 7 + flameIndex) * 0.13,
      0.24 + flicker * 0.22,
      length * along,
    );
    child.rotation.y = animationTime * 1.8 + flameIndex;
