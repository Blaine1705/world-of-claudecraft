...
export async function prewarmPlayerPortrait(
  ...
  await runPortraitPrewarm<CharacterVisual>({
    ...
    uploadTextures: async (visual) => {
      ...
      await uploadTexturesInSlices((prewarmRig as PortraitRig).renderer, textures, {
        yieldToMain: yieldToMainThread,
        // A graphics rebuild mid-sweep disposes the rig (rig goes null).
    ...
    compile: (visual) => {
      const r = prewarmRig as PortraitRig;
      r.mount.add(visual.root);
      const compiled = r.renderer.compileAsync(r.scene, r.camera);
      r.mount.remove(visual.root);
    ...
    renderAndSnapshot: (visual) => {
      const r = prewarmRig as PortraitRig;
      renderPortraitFrame(r, visual, visualKey, framing);
      return encodePortraitPng(r.renderer.domElement);
    },
    release: (visual) => {
      (prewarmRig as PortraitRig).mount.remove(visual.root);
      visual.dispose();
    },
