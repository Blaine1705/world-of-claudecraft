...
- Form adornments: `form_adornments.ts`, the per-rig owner `CharacterVisual`
  ...
  `form_adornment_textures.ts`. Pieces ride the rig's `head`/`chest` bones,
  carry the `weaponVfxMesh` marker so no overlay swap, prewarm twin or caster
  sweep touches them, and their shared kits are prewarmed through
  `ABILITY_MATERIAL_SOURCES` (`tests/form_adornments.test.ts`,
  `tests/character_form_adornments.test.ts`).
