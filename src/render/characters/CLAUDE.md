...
- Appearance decals/motion: `stubble.ts` (own section below), `makeup.ts`
  ...
  a row band per unit; the renderer holds a live candidate until they are
  resident, never under a cover or for the target: the producer contract in
  `src/render/CLAUDE.md`), `hair_sway.ts` (long-hair motion
