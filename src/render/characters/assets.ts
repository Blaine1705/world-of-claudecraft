...
export function tintedMaterial(
  ...
  const key = `${src.uuid}|${tint ?? 'n'}|${tint === null ? 0 : strength}|${GFX.standardMaterials ? 's' : 'l'}|${skinTex ? skinTex.uuid : 'n'}|${emisTex ? emisTex.uuid : 'n'}|${role}|${mount}|${selfIllumination}|${envMapIntensity ?? 'n'}`;
  const build = () =>
    buildTintedClone(
