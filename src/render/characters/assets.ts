...
function applyLowReadabilityLift(
  ...
  if ((mat as THREE.MeshLambertMaterial).isMeshLambertMaterial) {
    ...
    // An authored VERTEX-coloured surface (the harbormaster's gear) has no map to scale the
    // floor through, and three never multiplies emissive by vertex colour: the uniform floor
    // would film its dark felt grey. Its albedo lives in the vertices, so it takes no floor.
    else if (authored && lambert.vertexColors) lambert.emissive.setScalar(0);
  }
}
