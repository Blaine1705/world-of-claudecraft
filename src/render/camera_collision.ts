...
/**
 * Smooth third-person camera collision while preserving a hard safety limit.
 *
 * `hardLimit` is the closest legal camera fraction from the real collider
 * sweep; the returned physical `pullT` never exceeds it. When a new occluder
 * forces an immediate clamp, `lensT` and `fov` ease the perceived zoom without
 * moving the camera into geometry.
 */
