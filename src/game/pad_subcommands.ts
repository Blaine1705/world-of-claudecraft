...
//
// It does not build a menu: the HUD already opens one on right-click, for the
// target frame, the player frame and each party row. This synthesises that same
// right-click at the frame's own position, so the pad and the mouse can never
// drift into offering different options for the same target.
//
// Keyboard and mouse never reach this; only the pad's subcommand button calls it.

// The frames that carry a context menu, in the order the pad should prefer them:
// the current target first, since that is what the button is asking about.
