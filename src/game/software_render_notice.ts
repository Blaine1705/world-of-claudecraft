// Boots the one-time GPU notice once the renderer exists: combines the
// adapter-name verdict resolved during initGfxTier with the drift-proof
// failIfMajorPerformanceCaveat probe (either firing means the session is on a
// software rasterizer), folds in whatever the desktop shell has already said
// about the machine's GPUs (src/game/desktop_gpu_status.ts), and hands the
// result to the UI toast. Lives in src/game so main.ts stays a firewall
// (composition only) and neither ui nor render has to import the other.
...
export function initSoftwareRenderNotice(desktopShell: boolean): void {
  ...
  initGpuNotice({ ...verdict, desktopShell });
}
...
/** True when the notice showed the SOFTWARE-rendering copy this session. */
export function softwareNoticeShown(): boolean {
  return gpuNoticeDisplayed().softwareRendering;
}
