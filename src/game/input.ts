...
import { dropPointerFocus } from '../ui/pointer_blur';
...
type ButtonFocusTarget = {
  tagName?: string;
  blur?: () => void;
  closest?: (selector: string) => unknown;
};

function isButtonFocusTarget(
  active: ButtonFocusTarget | null,
): active is ButtonFocusTarget & { blur: () => void } {
  if (!active) return false;
  return active.tagName?.toLowerCase() === 'button' && typeof active.blur === 'function';
}
...
export class Input {
  ...
  // A native <button> (action-bar slot, bag row, sidebar icon...) keeps browser
  // focus after a mouse click. Left focused, it hijacks the very next Space or
  // Enter meant for jump/chat: the browser replays that still-focused button's
  // own native Space/Enter click activation, re-using whatever it does
  // (dismounting, re-consuming a potion...) instead of, or alongside, the
  // intended game action. Shed focus right after a MOUSE-driven activation so
  // the next keypress reaches gameplay untouched.
  //
  ...
  private releaseMouseActivatedFocus(e: { type: string; detail?: number }): void {
    ...
    const active = document.activeElement as ButtonFocusTarget | null;
    if (!isButtonFocusTarget(active)) return;
    dropPointerFocus(active);
  }
