...
// inside the Ignivar raid. Geometry comes straight from DungeonLayout: authored
...
import { IGNIVAR_GATE_LOCKED_TEMPLATE, isIgnivarRaidRoom } from '../sim/ignivar_raid_ids';
import { authoredWallSegments } from '../sim/rift/authored';
import { NPC_INTEREST_RADIUS, PLAYER_INTEREST_RADIUS } from '../sim/types';
...
/** Resolve the mapped raid room and instance-local player position from world coordinates. */
export function dungeonMapLocal(x: number, z: number): DungeonMapLocal | null {
  const frame = dungeonInstanceAt(x, z);
  if (!frame || !isIgnivarRaidRoom(frame.dungeonId)) return null;
  return {
...
/** Player-centred raid plan for the circular minimap. */
...
/** Whole current raid-room plan fitted uniformly into the square M-map. */
