...
// the "town done" line once every town quest (sim/town_quests.ts) is turned
// in, nothing otherwise. The decision itself is the sim leaf's zoneEntryHint;
// this core only maps the hint to localized text, so the HUD stays a one-line
// consumer and the rule is testable without a DOM. Pure core: no DOM, no
// Three; the world dependency is the two members it reads.
import { zoneEntryHint } from '../sim/town_quests';
import type { PlayerClass, QuestState, ZoneDef } from '../sim/types';
import { zoneWelcome } from './entity_display_core';
...
export function zoneEntryLine(zone: ZoneDef, world: ZoneEntryWorld): string | null {
  ...
  if (hint === 'town_done') {
    return t('hudChrome.zoneEntry.townQuestsDone', { zone: zoneDisplayName(zone.id) });
  }
