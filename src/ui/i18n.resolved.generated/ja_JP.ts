...
export const ja_JP: EnTranslations = {
  ...
  "hudChrome": {
    ...
    "worldPvp": {
      ...
      "groundFfa": "レイスウッド、エバーガーデン、ナイトブルームは無差別戦闘地帯で、フラグの有無に関わらず誰でも戦えます。",
      "groupLine": "パーティ、レイド、ギルドのメンバー同士が敵対することはありません。",
    ...
    "hill": {
      ...
      "moves": "{minutes} 後に移動します",
      "honorNote": "円内にいる占拠側の各メンバーに毎分 名誉 {honor}、同時に最大 {max} 人まで"
    },
    "warfareShop": {
  ...
  "guide": {
    ...
    "worldPvpPage": {
      ...
      "introZones": "オープンワールドの対人戦は任意参加で、その意味は足元の土地が決めます。PvPフラグを立てると、パーティ、レイド、ギルドに属さないフラグを立てた全プレイヤーが係争地で敵になります。フラグを下ろすと、少し経てば再び傍観者に戻ります。2つのゾーンはワールドの戦闘が一切起こらない聖域で、レベル上限のゾーンのうち3つは、フラグの有無に関わらずその場の全員が攻撃対象となる無差別戦闘地帯です。パーティ、レイド、ギルドの仲間はどこでも敵になりません。",
      "zonesHeading": "ワールドPvPが起こる場所",
      "zonesBody": "世界の土地は3種類に分かれます。修練の浜とイーストブルック渓谷は聖域で、フラグの有無に関わらずワールドPvPは一切起こらないため、新しいキャラクターがフラグとは何かを知る前に戦いを挑まれることはありません。世界の大半は係争地で、そこでは上記のフラグの規則がすべてです。レイスウッド、エバーガーデン、ナイトブルームは無差別戦闘地帯で、そこにいる全員がフラグの有無に関わらず、そこにいる他の全員を攻撃できます。境界を越えて入るときにも、出るときにも通知されます。そこでフラグを立てていないプレイヤーを攻撃すると自分のフラグが立つので、仕掛けた側が必ずリスクを背負います。すでにフラグを立てている相手を攻撃しても自分のフラグは立たないため、自衛や、フラグを立てていない誰かを守ることには何の代償もありません。",
      ...
      "hillHeading": "丘の王",
      "hillBody": "1時間に1度、無差別戦闘地帯のいずれかに丘が現れます。開けた土地に描かれた広い円で、その出現はレルム全体に告知され、立っている場所が地面にも示されます。円の中に立つプレイヤーが最も多いグループが丘を争い、1分間途切れずに多数を保てば丘はそのグループのものになります。パーティやレイドは一つのグループとして数え、単独のプレイヤーも一人のグループです。グループが丘を占拠している間、円の中に立つそのメンバーは毎分わずかな名誉を得ますが、同時に受け取れるのは数人までです。そのため満員のパーティが誰にも争われずに1時間占拠し続けると、バトルグラウンドの勝利1回分とほぼ同じ名誉になります。フィールド上部のバーが、誰が占拠しているか、あなた側と相手の人数、そして争奪の時計を表示します。チャットで /hill と入力すると丘の場所が分かります。",
