...
export const ja_JP: EnTranslations = {
  ...
  "guide": {
    ...
    "reliquaryPage": {
      ...
      "howBody": "ゲーム内で聖遺物庫を開きます（既定は Shift+X）。各棚にユニークな聖遺物のページがあります。キャラクターが初めてその品を得るとシルエットが埋まり、ページ上のすべてが埋まると点灯します。ライブの発見はトーストと開いているウィンドウの更新になります。進捗はキャラクター単位で、武器スキンだけはアカウントの見た目です。",
      "ranksHeading": "キュレーターランク",
      "ranksBody": "キュレーターランクはカタログに載せたユニーク埋めで上がり、装飾の称号と枠だけを与えます。戦闘力、ドロップ率、天井はありません。アカウントの武器スキンはキュレーターランクに数えず、名声はキャラクターに残ります。",
  ...
  "editor": {
    ...
    "status": {
      ...
      "campCapReached": "キャンプの上限に達しました（{max}）。新しいキャンプは追加されませんでした。",
  ...
  "loading": {
    ...
    "tips": {
      ...
      "reliquary": "ヒント：聖遺物庫は集めたユニークな戦利品を記録します。既定では Shift+X で開きます。"
    }
  },
  "errors": {
