...
export const ja_JP: EnTranslations = {
  ...
  "entities": {
    "abilities": {
      ...
      "lava_burst": {
        "name": "マグマバースト",
        "description": "{damage}の火炎ダメージを与える。あなたの灰燼の衝撃で燃えている目標には必ずクリティカルヒットする。マグマサージ：灰燼の衝撃の継続ダメージごとに20%の確率でこのクールダウンをリセットし、次のマグマバーストを即時詠唱にする。ダメージは呪文威力とともに上がる。（サンダーコール）"
      },
      "thunderstorm": {
        "name": "ストームブレイク",
