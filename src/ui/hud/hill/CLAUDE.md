...
The bar over the standing hill (`src/sim/pvp/hill.ts`, read through
`IWorld.hillInfo`), behind the `index.ts` barrel:
...
- `hill_bar_view.ts`: the pure, DOM-free core (registered in `UI_PURE_CORES`).
  ...
  while the local player stands in the hill's zone (`info.inZone`); the holder
  ...
  until the hill moves; and the structural `sig` (zone, geometry, holder,
  challenger, inside) the painter rebuilds its skeleton on.
- `hill_bar_painter.ts`: the thin painter (`HillBar`), a self-mounted
  ...
  scoreboard's slot. ONE innerHTML write per sig change; every per-second
...
- The circle itself is drawn by the renderer (`src/render/hill_ring.ts`); the
  `/hill` chat readout and the rise announcement come from the sim through the
  matcher (`src/ui/sim_i18n.ts`, `hill.*`).
