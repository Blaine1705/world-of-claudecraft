...
- **HUD-chrome WCAG 2.2 AA contract.** The chrome (windows, buttons, forms, menus, chat,
  ...
  - **Focus management:** opening a window TRAPS Tab/Shift+Tab inside it and RETURNS focus to
    ...
    wired by `src/ui/chrome_focus_wiring.ts`), so it records no stale trigger (a rail click
    blurs to the body; a click inside a dialog-rooted window parks focus on that window's
    root, which keeps the trap armed and is never a control to restore: `focusedWithin`
    refuses the root itself). Two modality discriminators coexist on purpose: `UIEvent.detail`
    for click activation (this), and the pointerdown flag for focus-driven tooltips
    (`hud.ts`); pick by what the handler receives, a click or a focus event.
