  ...
  profPages: {
    ...
    craftProse: {
      ...
      engineering: {
        ...
        materialsBody:
          'Every tool recipe consumes the tool one tier below it plus a FINE material, and that pairing is the whole ladder: four Fine Iron Ore and a Skysilver Mining Pick become the Osmium Mining Pick, then two Glyphsteel Bars, two Fine Osmium Ore and that osmium pick become the Glyphsteel Mining Pick. The axe and sickle lines mirror the same shape with Fine Ashwood and Fine Highpine Logs, Fine Goldleaf and Fine Sunpetal Herbs.\n\nA fine material is not sold anywhere and does not drop from an ordinary harvest: you get it by working a vein with a tool ranked above the material itself, which in practice means the tool one rung below the one you are trying to build. That is deliberate. The route to a tier 5 tool runs through actually swinging the tier 4 one, not through a shopping trip. The single exception is the Glyphsteel Bar, refined and vendor-only from Quartermaster Bree in Highwatch, so every glyphsteel-tier tool carries a fixed coin floor built into its cost.',
