...
export const zh_TW: Partial<Record<TranslationKey, string>> = {
  ...
  'entities.quests.q_ps_strike_true.text':
    '歡迎來到試煉之濱，{playerName}。這裡的每一課都從同一件事開始：站穩腳，握好刃。營地西南練武場上的草人生來就是挨打的。要是找不到練武場，按 M 打開地圖：你身上的每件差事都會標在上面，用不著亂繞。走到一個草人跟前，按 Tab 或用滑鼠左鍵點擊它，把它設為目標，接著按 1，或點擊畫面底部動作列上的第一個圖示，揮出這一刀。一直打到三個散架為止。它們的還擊不痛不癢。這片海濱之外的東西下手可重得多。',
