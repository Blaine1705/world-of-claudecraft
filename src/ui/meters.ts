  ...
  onEvent(ev: SimEvent, world: IWorld, partyPids: Set<number>, now: number): void {
    ...
    if (ev.type === 'damage') {
      ...
      if (targetInParty && ev.kind === 'hit' && (ev.amount > 0 || (ev.absorbed ?? 0) > 0)) {
        ...
        const lethal = hpAfter <= 0 || targetEntity?.dead === true;
    ...
    if (totalGroupAmount > 0) {
      ...
      if (this.settings.showRaidTotals && !isThreat) {
        if (this.tab === 'heal') {
          ...
          summaryText += ` · Grupo: ${fmtPerSecond(totalRaidDmg / dur)} DPS`;
        } else if (this.tab === 'dmg') {
          const totalRaidHeal = [...enc.tallies.values()].reduce((sum, t) => sum + t.heal, 0);
          summaryText += ` · Grupo: ${fmtPerSecond(totalRaidHeal / dur)} HPS`;
        }
      }
    }
    if (this.selectedPhase) {
      summaryText += ` [Fase: ${this.selectedPhase}]`;
    }
