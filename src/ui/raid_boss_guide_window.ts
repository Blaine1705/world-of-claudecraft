// Cold DOM adapter for the raid guide opened from the party-frame footer.
...
export class RaidBossGuideWindow {
  ...
  constructor(
    ...
    this.button.addEventListener('click', (event) => {
      // Pointer activation must not leave a stale chrome control focused after the
      // guide closes; keyboard activation deliberately keeps the opener for return.
  ...
  syncAvailability(dungeonId: string | null): HTMLButtonElement | null {
    ...
    if (!boss) {
      if (this.isOpen) this.close(this.deps.contextFallback());
        ...
        // PartyFramesPainter removes this node immediately after the null return.
        // Move keyboard focus to stable HUD chrome before that detachment.
