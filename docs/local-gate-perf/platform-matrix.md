...
| Full-suite lock `scripts/lib/gate_lock.mjs` | **smoke** (unit pins OS-agnostic; no macOS host this packet) | **verified** (unit pins plus a real two-process contention and killed-holder reclaim run on Linux) | **smoke** (unit pins OS-agnostic; `fs`'s `wx` create flag and `process.kill(pid, 0)` liveness probe are both implemented the same way on every Node-supported platform, no Windows host this packet) | Advisory `os.tmpdir()` lock serializing only the `vitest (full suite)` step across concurrent local gates; `GATE_NO_LOCK=1` opts out. No platform-specific locking primitive (issue #2808's acceptance criteria). |
