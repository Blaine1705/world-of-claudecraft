...
floor, each a real mob the party can kill, and until they are cut down each one
alternates two attacks from where it rose: a LINE WHIP along a lane and a
CIRCULAR SWEEP round itself.
...
Every cue is an ordinary hoard cue, and every one is a SWEEP, so the boss
engine's busy gate holds his Crashing Tide back while a tentacle stands. The
set only rises into a clean room, never into the middle of a set of his waves.

```
tide-tentacle        0 ... spawnWarningSec ............. + lifeSec
...
tide-tentacle-fall   the SAME cue id, retractSec long: killed (halfAngle 1, it
                     thrashes, drops and rots away) or out of time (0, it
                     withdraws)
...
Which one a tentacle makes alternates (`tentacleAttackKind`), neighbours out of
step. A sweep with nobody near becomes a lash instead.

## Pressure and safe space

Below `HOARD_DOUBLE_MECHANIC_INTENSITY` only ONE telegraph is ever down at a
time, whatever the number of tentacles. At it (legendary with four or more,
epic with five), two may be down together: always one of each kind, the second
at least `doubleStaggerSec` after the first.
