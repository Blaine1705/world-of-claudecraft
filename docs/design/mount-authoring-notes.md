...
6. Item icon art at `public/ui/items/reins_<id>.webp` + a provenance row in
...
shared `meshopt()` step corrupts it (this happened here — multi-primitive skinned
...
environment" — WRONG, it downloads fine directly from the GitHub releases page (Linux
x86_64 tarball, no sudo needed, just extract and add `bin/` to PATH). Don't accept "the
compression tool isn't available" as a permanent blocker without actually trying a direct
download first.
...
- If the material system evolves after the spec is first authored (vertex-color-only ->
...
- Follow `docs/design/item-icon-art-style.md` (contract id `woc-item-icon-v1`) for the
  ...
  deep shadow, centered subject at 68-76% fill, no UI border/text/watermark/transparency.
  For a mount specifically, the "Mount collectible" family row wants "a recognizable
  three-quarter mount bust or vehicle portrait with tack and personality," explicitly NOT
  just loose reins.
...
owned/total catalog counts AND its slot-total count AND a "three slots"->"four slots"
...
- **`git checkout --ours`/`--theirs` semantics SWAP between rebase and merge.** During a
  rebase, `--ours` is the branch being rebased ONTO (upstream), `--theirs` is the commit
  being replayed (your own change) — the opposite of what a merge means. When in doubt,
  use an unambiguous `git checkout upstream/<branch> -- <file>` instead of relying on
  ours/theirs at all.
...
- **A tool/environment mismatch can look like a code regression.** Re-rendering all 230
  ...
  currently-committed images produces small-but-real pixel drift (~0.7/channel average)
  across EVERY portrait, even on a completely unmodified checkout. Isolate this by testing
  against a clean, untouched base before assuming a fingerprint mismatch means your branch
  broke something — it might mean your tooling doesn't match whoever generated the
  baseline, which is a different problem with a different (and possibly undoable-by-you)
  fix.
