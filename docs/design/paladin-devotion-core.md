...
Devotion ranges from 0 to 20. Holy gains it from healing casts and from unleashing an
...
internal cooldown. Devotion never decreases with time. Death still resets the resource.
...
| Mending Light / Luz Reparadora | Basic directed heal. At rank 4 it heals for 190 to 222 and costs 55 mana. | Holy: 1 on a healing cast. Other specs: 0. | No cooldown, 2.5 sec cast, GCD | 30 yd | Never consumes a charge and is not empowered. | A narrow beam that gathers into the ally. Holy filler when stronger heals are unnecessary. |
...
| Avenging Wrath / Ira Vengadora | All specializations. Grants 10 Devotion, doubles Devotion generated by abilities for 15 sec, and increases damage by 20%. No mana cost. | 10 immediately, then double ability generation | 120 sec, instant, off GCD | Self | Never consumes a charge. Retribution may use Hammer of Wrath against any target while active. | Holy wings mark the accelerated build window. Holy doubles healing generation, while Protection and Retribution double damaging ability generation. |
...
| Solar Invocation | Holy only. Instant, 8 sec cooldown, 80 mana, and 30 yd range. Heals one friendly target for 180 to 220 and generates 1 Devotion on a valid healing cast. During Ascension it consumes 1 charge and also heals allied players within 10 yd of the target for half as much. |
...
Devotion Ward and Requital Aura form the active aura choice: casting one replaces only the
other aura owned by that Paladin. Radiant, Dawn, and Grace Devotion are standalone spells.
They coexist with the active aura and with each other.

| Devotion | Duration | Effect |
| --- | ---: | --- |
| Devotion Ward | Until replaced | 5% damage reduction. |
| Requital Aura | Until replaced | Returns Holy damage when an affected ally is struck. |
...
Holy generates Devotion through valid healing casts and by unleashing an active Seal with
...
| --- | --- | ---: | --- | ---: | --- | --- |
| Mercy Lance / Lanza de Misericordia | Heals an ally or damages an enemy for 80 to 100. Costs 20 mana. | 1 on a healing cast | 6 sec, instant, GCD | 30 yd | Marked. On a healing cast it consumes 1 charge and also heals one nearby ally at 70% power. | A narrow lance changes from warm gold on allies to white-gold on enemies. Use frequently for movement and efficient spot healing. |
| Dawn's Embrace / Abrazo del Alba | Strong heal for 260 to 310. Costs 45 mana. | 2 on a healing cast | 10 sec, 1.5 sec cast, GCD | 30 yd | Marked. Consumes 1 charge, becomes instant, and heals for 35% more. | A sunrise fan opens behind the target. Tank or emergency heal and the strongest reactive charge use. |
| Radiant Chorus / Coro Radiante | Heals nearby allies for 90 to 110 in 30 yd. Costs 60 mana. | 2 if at least one eligible ally is reached | 12 sec, 2 sec cast, GCD | Self-centered | Marked. Consumes 1 charge, heals for 20% more, and expands to 40 yd. | Several light notes or rings converge on injured allies. Group damage response. |
...
- Holy reaches 20 in 36.15 seconds during uninterrupted healing casts with its real mana
  pool. Mending Light
...
1. Verify state correctness: generation, 20-point gate, five charges, one charge per marked
   cast, 25 second expiry, death reset, time-stable Devotion, and 10-point active bank.
