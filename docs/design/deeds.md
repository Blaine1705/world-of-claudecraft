...
| Border | A cosmetic frame accent on capstone deeds that a player can select and wear (one at a time, chosen in the Book of Deeds beside the title picker): an in-world cartouche on the nameplate (shared well and hardware, per-slug side motif), the same family on the player and target portrait rings, plus a badge on the character sheet. |
