...
- **The CraftPix class ability icons** (`public/ui/skills/<class>/*.webp`, limited to
  entries whose per-class `mapping.json` record names a CraftPix source pack and does not
  carry an overriding project-generated or owner-provided licence). These were **purchased by the Levy Street account**
...
| Generated item icon rebrand (`public/ui/items/*.webp`, exact IDs under `generatedBatches` in `public/ui/items/mapping.json`) | World of ClaudeCraft | Project-generated with OpenAI image generation, reviewed and optimized locally; the professions-tuning rods also used the licensed CraftPix Premium `simple_fishing_pole` icon as a style reference, as recorded in the mapping | Project asset, rights reserved | **No, permission required** |
...
| Generated class ability additions (`public/ui/skills/<class>/*.webp`, exact `woc_openai_missing_painted_icons_2026_08_01` entries in each class's `mapping.json`) | World of ClaudeCraft | Project-generated with OpenAI built-in image generation from the exact repository references recorded per ability; reviewed, normalized, and optimized locally; prompt, reference-role, source-hash, and shipping-hash record in `docs/achievements/missing-painted-icons-accepted-art.json` | Project asset, rights reserved | **No, permission required** |
...
| Book of Deeds achievement icons (`public/ui/deeds/*.webp`, excluding the eight generated additions listed next; downscaled from reviewed 512px sources to 128px WebP via `scripts/convert_deed_icons_webp.mjs`) | World of ClaudeCraft | Maintainer-commissioned bespoke art, owned by the project | Project asset, rights reserved | **No, permission required** |
| Generated Book of Deeds additions (`chr_peaks_gatherer`, `chr_marsh_rares_ii`, `chr_peaks_rares_ii`, `chr_gleamstag`, `chr_hollow_rares`, `dgn_wildheart_basin`, `dgn_wildheart_basin_heroic`, and `pvp_card_duel_first_win` under `public/ui/deeds/`) | World of ClaudeCraft | Project-generated with OpenAI built-in image generation from the exact project-owned, project-generated, commissioned, and licensed CraftPix Premium repository references recorded for each crest; the earlier five include the licensed CraftPix Premium `grubjaw_tusk` and `old_cragmaws_pelt` item references and retain their prompt/lineage record in `docs/achievements/professions-tuning-art-provenance.md`; the final three are recorded in `docs/achievements/missing-painted-icons-accepted-art.json`; all were reviewed, keyed, centered, and optimized locally | Project asset, rights reserved | **No, permission required** |
