...
| Bank-storage bag catalog icons (`public/ui/items/{wayfarers_backpack,burlap_reagent_pouch,necromancers_reagent_satchel,duskweave_bag,resonant_weave_bag,foragers_haversack,loombound_reagent_satchel}.webp`, the `woc_original_svg` overriding-licence entries in `public/ui/items/mapping.json`) | World of ClaudeCraft | Project-generated procedurally rasterized SVG placeholders, pending a commissioned painted icon in the `woc_bags` style; provenance per icon in `public/ui/items/mapping.json` | Project asset, rights reserved | **No, permission required** |
