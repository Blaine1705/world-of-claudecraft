...
| Tank rideable mount model and item icon (`public/models/mounts/tank.glb`, `public/ui/items/reins_tank.webp`) | World of ClaudeCraft | Original project art procedurally reconstructed from a user-supplied concept without redistributing source pixels; deterministic source, design boundary, and provenance are committed under `scripts/assets/tank_mount/` and `docs/design/tank-mount/` | Project asset, rights reserved | **No, permission required** |
...
| Tank movement layer (`mount_run_tank.mp3`) | World of ClaudeCraft | Original deterministic FFmpeg synthesis via `scripts/gen_tank_mount_sfx.mjs` | Project asset | With the project only |
