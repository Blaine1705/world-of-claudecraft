...
| Proving Shore candidate cues (`docs/music/proving-shore-candidates/*.mp3`, the audition decision record; the chosen candidate also ships as `public/audio/music/proving_shore.mp3`) | World of ClaudeCraft | Composed in the project's procedural score (`src/game/music_themes_proving_shore.ts`), rendered via `scripts/render_music.mjs`, mastered locally with ffmpeg/sox | Project asset | With the project only |
