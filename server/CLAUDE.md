...
| `jump_latch.ts` | pure server-side counterpart to the client's momentary-jump latch (`src/game/input.ts` `KEY_JUMP_LATCH_MS`): `noteJumpInput`/`resolveJumpInput` keep a momentary jump press from being last-write-wins overwritten by its own release frame when jittery delivery bunches both onto the same inter-tick window. `dispatchMessage`'s `input` arm records the raw flag; `GameServer.applyJumpLatches` (called every tick, right before `Sim.tick()`) is the only writer of `PlayerMeta.moveInput.jump` derived from it |
