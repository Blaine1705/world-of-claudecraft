...
| `woc_market.ts` (+ `woc_market_rules.ts`/`woc_market_db.ts`/`woc_market_routes.ts`/`woc_market_proxy.ts`/`woc_market_custody.ts`/`woc_market_sweep.ts`/`woc_market_monitor.ts`) | the $WOC Exchange (docs/prd/woc/marketplace.md): USD-cent auctions settled in $WOC. Pure rules core / schema + atomic transition SQL / RouteDef surface + operator arms / economy-service client with the dev-gated in-memory arm / the one bridge into the live Sim (escrow extraction, custody mail) / the per-realm advisory-locked sweep / the stuck-custody monitor. Config-gated off (`WOC_MARKET_ENABLED`); the game computes no token math. Delivery is exactly-once by construction: the close tail commits as ONE transaction (`finalizeDeliveredSettlement`; the minute-scale `redriven` beat converges an older binary's delivered-but-unclosed and sold-but-undisposed residue forward over bounded id pages), the claims ledger attributes every ref to a rail (`grant_character_id` / `mail_intent_at`) and a resume needs PROOF: a mail claim re-mails only while its parcel is still in the live book (or this process's own attempt), a grant claim retries only its own live session, and anything unprovable (both intents NULL, a collected letter, a lease fence, a restart) PARKS, since a fence rejection cannot disprove an earlier autosave. The direct hand-off books atomically with its fenced character save (`saveDeliveredCharacterBooked`). Sweep arms are error-isolated (`onSweepError`), delivery stats count rows ADVANCED, and parked rows rotate (`touchSettlementRow`) with an in-process minute backoff; anything parked is surfaced by `woc_market_monitor.ts`, one cached readout (counts SATURATE at a cap) behind `GET /internal/woc-market/stuck` (dashboard secret, `server/internal.ts`) plus a minutes-scale log beat that runs even while the market is disabled. LOCK ORDER for a market transaction that touches bid rows AND the listing row: bids first (`suspendListingIfSafe` pre-locks the open set PLUS 'won' by id, `activateBid` the open set; `insertSettlement` stamps its one winner bid; `finalizeDeliveredSettlement` pre-locks the open set plus the winner), listing second; the reverse order deadlocks. A transaction that takes no bid row lock documents its carve-out in place (`cancelListingIfUnbid`, `insertPendingBid`, `escrowInsertListing`, `saveDeliveredCharacterBooked`). The guard transactions bound their waits with `SET LOCAL lock_timeout` and surface 55P03/40P01 as the typed `contended` refusal. Pins: the DB-free structural floor in `tests/server/woc_market_directed_sql.test.ts` always runs; the interleaves in `tests/woc_market_settlement_pg_integration.test.ts` and the delivery crash-point matrix in `tests/woc_market_delivery_pg_integration.test.ts` need `TEST_DATABASE_URL`. Its settlement-guard unique indexes deliberately ride boot DDL, not `concurrent_indexes.ts` (pre-enable-empty tables; rationale at the DDL) |
