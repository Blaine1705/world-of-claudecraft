...
| `movement_stop.ts` | validates a client's release endpoint against the immediately previous/next authoritative movement segment and owns the bounded prepare/finish tick handoff; it never accepts a free position or increases server movement speed |
| `movement_input_timeline.ts` | protocol-v2 input timeline: applies client-timestamped movement transitions after the shared bounded jitter window; old clients bypass it |
| `movement_position.ts` | validates display-position streams against elapsed movement-speed budget, traversal collision, server-owned locomotion locks, and a short authority window before adopting grounded endpoints into the authoritative sim |
