...
// server/http/config.ts; unset keeps forever, the fail-safe default).
...
/** The closed FTUE event vocabulary. A new kind is appended here and in the
 *  DDL CHECK below in the same change. */
...
// mirroring unstuck_reports (both are sparse, low-volume telemetry tables
// created by the boot DDL transaction).
...
CREATE TABLE IF NOT EXISTS ftue_events (
  ...
  CONSTRAINT ftue_events_kind CHECK (
    kind IN ('quest_accepted', 'quest_done', 'death')
  )
);
 ...
 *  FTUE_MAX_LEVEL gate; this stays a plain parameterized insert. */
