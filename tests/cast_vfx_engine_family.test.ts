...
describe('the rest of the renderer cast VFX', () => {
  for (const lowDetail of [false, true]) {
    it(`stays out of the gate with its units kept (${lowDetail ? 'low' : 'full'} detail)`, () => {
      ...
      // pools' own counts (tests/class_vfx_prewarm_homes.test.ts) minus the
      // programs they share with the engine and the kit.
      expect(collectAbilityVfxCompileTargets(scene).length - unitsBefore).toBe(lowDetail ? 8 : 11);
    });
  }
});
