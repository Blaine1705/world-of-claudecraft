...
describe('corpse decay over the wire', () => {
  ...
  it('suppresses stale loot advertisement once the corpse has decayed', () => {
    const mob = deadMob(9205, 0);
    mob.lootable = true;
    mob.lootFfaTimer = 0;
    mob.loot = { copper: 10, items: [{ itemId: 'wolf_fang', count: 1 }] };
    ...
    const w = wireEntity(mob);
    expect(w.cd).toBe(1);
    expect(w).not.toHaveProperty('loot');
    expect(w).not.toHaveProperty('ffa');
    expect(w).not.toHaveProperty('lootList');

    const client = bareClient(1);
    (client as any).applySnapshot({ t: 'snap', ents: [w] });
    const mirrored = client.entities.get(9205)!;
    expect(mirrored.lootable).toBe(false);
    expect(mirrored.loot).toBeNull();
    expect(mirrored.corpseTimer).toBeLessThanOrEqual(0);
  });
