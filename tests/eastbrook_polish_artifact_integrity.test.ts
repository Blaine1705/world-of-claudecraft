...
// mip-release branch: both sides move the rendererIntegration leaf, so both
// literals mint to values matching neither parent. No capture was retaken.
const ACCEPTED_POLISH_V2_METADATA_SHA256 =
  '940ac8a5514007629cd4757437cf1b3bd7ab1e38269d5fa358f42f96bacae725';
...
describe('Eastbrook polish performance and contact evidence', () => {
  it('pins the accepted matched performance evidence byte for byte', () => {
    ...
    // Re-pinned for the merge of release/v0.36.0 (post PR 3217/3219) into the
    // KTX2 mip-release branch: the first-order composite follows both parents'
    // rendererIntegration moves, then this second-order performance seal
    // follows the swept evidence bytes. No capture was retaken.
    expect(
      fingerprint.digest('hex'),
      `the second-order performance digest moved; if every input moved legitimately, re-mint with: ${REMINT_COMMAND} (it recomputes this literal LAST, from the swept files)`,
    ).toBe('e55346bd58b841ecf95f808c32ee9be9eca6abeb16beb8d4b6de4dd865bb434a');
  });
