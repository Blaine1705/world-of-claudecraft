...
describe('crafting window: Tab focus trap and restore-on-close', () => {
  it('installs the trap on open (captureFocus records the opener) and traps Tab within the window', () => {
    ...
    // Document order: the commission-orders button then Close (panel-title),
    // the craft's tab-strip button (.crafting-tabs), then the Craft row
    // button (.crafting-body).
    ...
    // and it wraps back to the first tabbable (the orders button) rather than
    // ever reaching the opener or body.
    ordersBtn?.focus();
