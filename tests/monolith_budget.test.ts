...
const MONOLITHS: MonolithRow[] = [
  ...
  },
  {
    ...
    // exact count measured on the MERGED working tree (wc -l < src/ui/hud.ts),
    // never reconciled by arithmetic. Zero slack.
    // LOWERED 18133 -> 18126 at the merge of release/v0.44.0 into the Eastbrook
    // ferry branch (the ferry HUD's three wiring lines ride the raid-marker label extraction): exact count measured on the MERGED working
    // tree (wc -l < src/ui/hud.ts), never reconciled by arithmetic. Zero slack.
    ceiling: 18126,
  ...
  },
  {
    ...
    // exact count measured on the MERGED working tree (wc -l < src/sim/sim.ts),
    // never reconciled by arithmetic. Zero slack.
    // LOWERED 11740 -> 11736 at the merge of release/v0.44.0 into the Eastbrook
    // ferry branch (the ferry wiring rides the camp_private_rng.ts extraction): exact count measured on the MERGED working
    // tree (wc -l < src/sim/sim.ts), never reconciled by arithmetic. Zero slack.
    ceiling: 11736,
  ...
  },
  {
    ...
    // LOWERED 9906 -> 9901 at the merge of release/v0.44.0 into the Eastbrook
    // ferry branch (the ferry's wire lines ride the lockpick action guard moving to server/lockpick_action.ts): exact count measured on the MERGED working
    // tree (wc -l < server/game.ts), never reconciled by arithmetic. Zero slack.
    ceiling: 9901,
  ...
  },
  {
    ...
    // never reconciled by arithmetic. Zero slack.
    // LOWERED 5416 -> 5392 at the merge of release/v0.44.0 into the Eastbrook
    // ferry branch (the ferry decode wiring rides the rest_types.ts extraction): exact count measured on the MERGED working
    // tree (wc -l < src/net/online.ts), never reconciled by arithmetic. Zero slack.
    ceiling: 5392,
  ...
  },
  {
    ...
    // merged tree. Exact merged count, zero headroom.
    // LOWERED 2548 -> 2513 at the Eastbrook ferry's Phase 2: the berth gate
    // bookkeeping (setColliderGateOpen, gridIndex-ordered reopen, lazy wishes) landed and
