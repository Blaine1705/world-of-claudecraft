...
describe('signed Pristine specimens (#1145)', () => {
  it('a rare-or-better harvest grants the signed specimen PLUS the plain component (seed 15)', () => {
  ...
  it('a below-rare harvest grants a plain stack at its tier quantity and NO specimen (seed 14)', () => {
    const { sim, internals, a, mob } = setup(14);
    sim.harvestCorpse(mob.id, ['hide'], a);
    const meta = internals.players.get(a)!;
  ...
  it('every other specimen family grants its own jackpot beside the plain component (seed 15)', () => {
  ...
  it('the cloth family (no specimen) grants the signed component at rare-or-better (seed 31)', () => {
  ...
  it('a slot-full specimen harvest truncates the specimen and keeps the plain yield (seed 15)', () => {
...
describe('corpse signed-guard capacity vs merge room (#2139)', () => {
  ...
  it('a slot-full specimen jackpot merges into a same-signer specimen stack instead of truncating (seed 15)', () => {
...
describe('a signed specimen-less grant carries its rolled quantity (#2473)', () => {
  ...
  it('a partial-merge spill can cost a pending specimen its slot, deliberately (seed 62)', () => {
    ...
    const stack = stackSizeOf(ITEMS.wolf_fang);
    const { sim, internals, a, mob } = setup(62);
    const m = internals.players.get(a)!;
    const cap = bagCapacity(m.bags);
...
describe('a repeated component tag harvests the family once (#2474)', () => {
  ...
  it('never mints a second signed Pristine Hide off one claim (seed 4, the issue case)', () => {
    ...
    // after the Galecrest quest-camp pass (#2887), then again after the
    // release sync shifted the camp-driven world-gen draw sequence: each time
    // the old seed stopped clearing the signable floor on both duplicate
    // rolls, or started clearing it on the deduped single roll, so it no
    // longer reached the doubled arm at all. Seed 4 does, and lands the same
...
describe('an invalid component tag is ignored entirely (#2504)', () => {
  ...
  it('an ALL-junk pick spreads, exactly as the empty pick does (the settled ruling)', () => {
    ...
    const junk = harvestWith('forest_wolf', ['junk'], 4);
...
describe('corpse premium-arm tool gating (Professions 2.0)', () => {
  ...
  it('bare hands still earn the signed specimen on real content: tier-1 families never gate (seed 15)', () => {
    const { sim, internals, a, mob } = setup(15);
    const meta = internals.players.get(a)!;
  ...
  it('a denied premium pull downgrades to the plain grant: same qty, same claim, same draws (seed 31)', () => {
    ...
    // quest-camp pass (#2887), then back to 31 after the release sync shifted
    // it once more. Every re-hunt reproduces
  ...
  it('at most ONE gatherDenied per harvest command, even with several denied families (seed 23)', () => {
    ...
    // quest-camp pass (#2887), then to 23 after the release sync shifted it
    // once more.
  ...
  it('the single event is tiered off the FIRST failing family in yield order (seed 23)', () => {
    ...
    // content pass shifted the camp-driven world-gen draw sequence again, then
    // from 62 to 280 after the Galecrest quest-camp pass (#2887), then to 23
    // after the release sync shifted it once more.
    const first = soloRig(23);
...
// as literals measured against a real Sim at seed 76.
//
...
describe('the concentration bonus on a mixed corpse, moved on purpose (#2514)', () => {
  function yieldOf(components: string[] | undefined) {
    const { sim, internals, a } = setup(76);
    const template = MOBS.old_greyjaw;
  ...
  // old_greyjaw, tags hide/fang/claw, seed 76. Every pick shape that yields
  ...
  // re-hunted for the zones 1-3 quest-dedupe content pass and again (115 to
  // 114) for the Galecrest quest-camp pass (#2887), since any content add
