...
describe('a corpse whose EVERY family is unmapped is never offered a harvest (#2513)', () => {
  ...
  it('every command that spends the claim reports at least one yield', () => {
    ...
    // trash templates their mapped harvest tags. The Drakelands/Willowfen/
    // Evergarden harvest-gap fix adds dune_troll (4 submasks), bogtoad (4
    // submasks, one of them the all-unmapped ['gills'] refusal), and
    // hedge_knight (2 submasks), so spent rises to 161 and refused to 17.
    expect(spent).toBe(161);
    expect(refused).toBe(17);
    expect(spent + refused).toBe(178);
  });
  ...
  it('every family a harvest extracts has an item behind it (#2514)', () => {
    ...
    // 228 with the Drakelands/Willowfen/Evergarden harvest-gap fix (dune_troll,
    // bogtoad, and hedge_knight add mapped-family submasks of their own).
    expect(extracted).toBe(228);
  });
