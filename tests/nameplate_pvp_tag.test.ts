...
// follows the entity's `pvpFlag` wire bit on the next full pass. Plate content
// is re-resolved on every full pass (nameplate_cadence_core.ts: 1/15 s on the
// LOW tier, 1/24 s above it), so a flag flip is on screen within one interval
// without the plate being marked urgent: an urgent plate re-resolves its
// content every frame, which is the cost of a live cast bar, not of a tag that
// changes a few times an hour. The idiom (fake canvas context, painter state
// access) is tests/nameplate_ai_tag.test.ts's.
...
describe('the nameplate <PvP> tag', () => {
  ...
  it('follows the wire bit on the next full pass, both ways, and leaves the colour to the hostility verdict', () => {
    const target = entity({ id: 2, name: 'Aleph' });
    let hostile = false;
