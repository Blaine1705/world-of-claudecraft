...
function view(): EntityView {
  ...
  return {
    group,
    height: 2,
    // the real EntityView always carries a saddle lift (renderer.ts); the
    // painter folds it into the anchor, so a fake view must declare it or the
    // plate height goes NaN and no transform is written at all
    mountLift: 0,
