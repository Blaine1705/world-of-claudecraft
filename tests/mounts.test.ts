...
describe('mount reins items (the collection: owning the item is owning the mount)', () => {
  ...
  it('every mount has exactly one reins item; player reins are unbound, the dev tank stays bound', () => {
    for (const key of MOUNT_KEYS) {
      ...
      if (key === 'terrorspark_groundshaker' || key === 'rickshaw_mount' || key === 'mech_bird') {
        // Three bound reins, for the same leak reason from different doors: the
        // developer-only tank and rickshaw have no player acquisition path, and
        // the store mount's reins is a real-money grant (server/claudium.ts).
        // Any of them trading hands would turn a grant into an economy leak.
