...
describe('Hud Warlock pet signature bar', () => {
  ...
  it('keeps only working controls for surviving Necromancy summons after Graveguard dies', () => {
    ...
    stanceMenu?.click();
    hud.renderPetBar(null);
    document.querySelector<HTMLButtonElement>('[data-focus-key="stance-passive"]')?.click();
    expect(hud.sim.setPetMode).toHaveBeenCalledWith('passive');

    expect(document.querySelector('[title="Heal Demon"]')).toBeNull();
    expect(document.querySelector('[title="Taunt"]')).toBeNull();
    expect(document.querySelector('[title="Felbolt"]')).toBeNull();
  });
