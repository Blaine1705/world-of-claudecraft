...
describe('missing painted deed and Heroic weapon integration', () => {
  it('leaves only the pinned art-pending deeds without painted art, and records generated crest ownership', () => {
    ...
    // profession deeds, and four Reliquary Curator rank bridges all appended after this
    // wave, so the live catalog is 251 and the wave's own claim is unchanged: every deed
    // that existed when it landed is painted. The only artless ids are those appended
    // later, which ride the category-crest fallback the Icons authoring rule in
    // docs/design/deeds.md sanctions, until their 512px sources are commissioned
    // (flagged in docs/achievements/icon-brief.md). Read from DEED_ART_PENDING, the one
    // enumeration of that debt (src/ui/icons.ts), so this file cannot end up naming a
    // different pending set than the other two art suites. Exhaustive: a third artless
    // deed still reds here.
    expect(DEED_ORDER).toHaveLength(251);
