...
describe('char_window: own-paperdoll per-copy tooltip threading (Phase 6)', () => {
  it('resolves the worn instance from the owner inventory facet inside the tooltip closure', () => {
    // The procedural-dungeons world API exposes the own worn set directly on
    // IWorldInventory. Read it per slot at hover time (a closure over
    // deps.world(), never a stale capture) and forward it into the widened
    // itemTooltip dep, preserving v0.28 maker marks alongside Rift payloads.
    expect(painter).toContain(
      'this.deps.itemTooltip(item, this.deps.world().equipmentInstances[slot])',
    );
  });
});
