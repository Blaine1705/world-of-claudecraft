...
// the exported statement FOR UPDATE inside its own bounded transaction and
// waits the in-flight transaction out. Only real PostgreSQL can prove the
...
d('the commit-ambiguity verify read against real PostgreSQL', () => {
   ...
   *  SET LOCAL bounds, the exported FOR UPDATE read, ROLLBACK. */
   ...
   *  until the verify backend really queues on the row lock. */
  ...
  it('waits the uncommitted delete out and answers landed once its COMMIT applies', async () => {
    ...
    try {
      ...
      // The exact wrong answer the FOR UPDATE exists to avoid: a plain
      ...
      let settled = false;
      const wait = runVerify(verifier).then((landed) => {
        settled = true;
        return landed;
      });
  ...
  it('answers not-landed with the surviving row when the in-flight delete aborts', async () => {
    ...
    try {
      await deleter.query('BEGIN');
      await deleter.query('DELETE FROM characters WHERE id = 42');
      const wait = runVerify(verifier);
      await waitForLockWaiter();
      await deleter.query('ROLLBACK');
      // The abort leaves the row: the verify locks it, reads it, and the
