...
describe('createWsAuth: authenticateWebSocket reject paths', () => {
  ...
  it('2c. rejects an auth-world-20 client on the auth-world-21 server before all admission work', async () => {
    const fixture = setup();
    const { ws, deps, req } = fixture;

    await createWsAuth(deps).authenticateWebSocket(
      asWs(ws),
      JSON.stringify({ t: 'auth-world-20', token: 'tok', character: 7 }),
      req,
    );
  ...
  it.each(['auth-world', 'auth-world-22', 'auth-world-next', 'auth-world-01', 'auth-world-1.0'])(
