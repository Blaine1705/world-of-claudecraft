...
describe('createWsAuth: authenticateWebSocket reject paths', () => {
  ...
  it('2c. rejects an auth-world-29 client on the auth-world-30 server before all admission work', async () => {
    const fixture = setup();
    const { ws, deps, req } = fixture;

    await createWsAuth(deps).authenticateWebSocket(
      asWs(ws),
      JSON.stringify({ t: 'auth-world-29', token: 'tok', character: 7 }),
      req,
    );
  ...
  it.each(['auth-world', 'auth-world-31', 'auth-world-next', 'auth-world-01', 'auth-world-1.0'])(
