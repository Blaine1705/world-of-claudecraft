...
describe('createWsAuth: authenticateWebSocket reject paths', () => {
  ...
  it('2c. rejects a release/v0.41.0 auth-world-9 client before all admission work', async () => {
    const fixture = setup();
    const { ws, deps, req } = fixture;

    await createWsAuth(deps).authenticateWebSocket(
      asWs(ws),
      JSON.stringify({ t: 'auth-world-9', token: 'tok', character: 7 }),
      req,
    );
  ...
  it.each(['auth-world', 'auth-world-11', 'auth-world-next', 'auth-world-01', 'auth-world-1.0'])(
