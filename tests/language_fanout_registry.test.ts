...
const NOT_A_LANGUAGE_GATE: ReadonlyArray<{
  ...
  },
  {
    ...
    memos: ['lastTradeSig'],
    reason:
      'the trade window repaint signature: it reads no text at all (offer structs, staged items and copper, acceptance flags, partner, the staged quote and consent structural state), so a locale switch cannot move it, and there is deliberately no fan-out arm, exactly as when the method lived on hud.ts. A live trade re-renders in the new locale on the next data motion (either offer, stake, or acceptance change; the standing-offer poll adoption; the 2s poll makes an ACTIVE deal converge within a beat); an idle open trade keeps the previous locale until then, the short-lived two-player surface this window is. RE-JUDGED by the UX-honesty pass, which added the consent row and the quote review to this arm: the posture stands because the consent SEND carries a boolean judged by the server, not the label text, and a live money flow always has data motion; the polish pass owns a relocalize() if the idle-window residue is worth the behavior change.',
  },
  {
    file: 'hud.ts',
    memos: 'coordinator',
