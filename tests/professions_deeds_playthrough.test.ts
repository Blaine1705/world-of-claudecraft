...
describe('scripted playthrough (one sim, live sites only)', () => {
  ...
  it('beat 11: the koi lands through the REAL bite-and-reel loop and the deed fires on the catch', {
    ...
    // Eastbrook camp respacing thinned the zone-1 camp counts): the koi bites on
    // session index 9.
  ...
  it('beat 15: the perfect specimen jackpot lands through the real corpse harvest', () => {
    ...
    // Hunted literal (seed 4242, after every beat above, re-recorded after the
    // Eastbrook camp respacing thinned the zone-1 camp counts): the rare-or-better
    // rarity roll that mints the signed specimen lands on attempt index 16.
    expect(hitAt).toBe(16);
