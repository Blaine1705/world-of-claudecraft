...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    ...
    // (data) + setActiveBorder (method), leaving 319. The backward
    // target cycle (Shift+Tab) adds tabTargetPrev (IWorldTargeting, a method),
    // leaving 320, and this branch's tutorial greeting adds startTutorial
    // (IWorldQuests, a method), leaving 321.
    //
    // NOTE for the next merge, five syncs run now: BOTH sides of this pin move
    // it independently every cycle. Twice git merged identical numbers with no
    // conflict while the real total was one higher; the later syncs differed so
