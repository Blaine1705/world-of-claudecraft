...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    expect(IWORLD_MEMBERS.length).toBe(242);
  ...
  it('the full sorted member set is exactly the pinned 242', () => {
...
//   (4) their union, sorted, equals the pinned 233-name IWORLD_MEMBERS set (a member
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 28 facets', () => {
  ...
  it('the union of the 28 facets equals the pinned 242-member IWORLD_MEMBERS set', () => {
    const union = Object.values(FACET_MEMBER_ARRAYS).flatMap((arr) => [...arr]);
    expect(union.length, 'union size before dedup (catches a duplicated member)').toBe(242);
    expect(new Set(union).size, 'union size after dedup (catches a duplicated member)').toBe(242);
