...
describe('selective gate planning', () => {
  ...
  it.each([
    ...
    // Nested helper/fixture dirs: a fixture-only edit under tests/server/
    // would otherwise classify as inert .json while its consuming suite can be
    // graph-visible, so the stale-expectation failure would be skipped.
    ['tests/server/fixtures/main/characterization.json'],
