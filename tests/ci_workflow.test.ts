...
describe('CI workflow parity', () => {
  ...
  it('bounds the test and browser jobs against the runner-side checkout-stall class', () => {
    ...
    const bounds = [
      ['pr-gate', 20],
      ['release-gate', 20],
      // Resized 20 to 40 with the nine-suite lane (review 3050): the five
      // class-overhaul balance harnesses joined after pushing two 20-minute
      // shards over budget, and the bound's rationale on the ci.yml job was
      // rewritten with it.
      ['pr-long-sims', 40],
      ['browser-gate', 10],
