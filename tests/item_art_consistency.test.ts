...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: '0cfc10c4e8c3034887c0c33aa29dc5cffc7149a452f38cca63492fb924b63d57',
        acceptedBytes: 111_167,
      },
    ]);
  ...
  it('pins the final item-art visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(111_167);
    expect(sha256(verdictBytes)).toBe(
      '0cfc10c4e8c3034887c0c33aa29dc5cffc7149a452f38cca63492fb924b63d57',
    );
    ...
    expect(verdict.auditScope).toMatchObject({
      ...
      itemArtFilesReviewed: 838,
      liveItemDefinitions: 853,
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: '49f7f271a1749f593004d68d24b622b1bcbd5ea6b6fe84f24b3742b69802d41a',
      bytes: 459_544,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      '7098c03f2186db8f5014689b346ba49a5b02bad3f1e36e242035539d9be245d7',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      '4f89334df6f4d5a53d8dfe65193d383c1df8e36586e5e874f510d8db085ecf67',
    );
  ...
  it('keeps an exact, non-dangling supersession graph and one current mapping owner', () => {
    ...
    expect(oldGeneratedIds).toHaveLength(524);
    const allCurrentOwnerIds = [
      ...mapping.entries.map(({ itemId }) => itemId),
      ...mapping.generatedBatches.flatMap(({ itemIds }) => itemIds),
    ];
    expect(allCurrentOwnerIds).toHaveLength(838);
    expect(new Set(allCurrentOwnerIds).size).toBe(838);
  ...
  it('keeps the full shipping icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    ...
    if (ownerIds.length !== 838) violations.push(`mapping owner count: ${ownerIds.length} != 838`);
    if (fileIds.length !== 838) violations.push(`shipping WebP count: ${fileIds.length} != 838`);
    for (const id of ids) {
