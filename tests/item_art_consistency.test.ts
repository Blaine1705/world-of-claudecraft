...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: 'a6b3de6c0a0581f2b9a4aab7a9b105c3e07cfbe47fd7aff4b0f17f0079b61695',
        acceptedBytes: 108_413,
      },
    ]);
  ...
  it('pins the final 817-item visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(108_413);
    expect(sha256(verdictBytes)).toBe(
      'a6b3de6c0a0581f2b9a4aab7a9b105c3e07cfbe47fd7aff4b0f17f0079b61695',
    );
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: '5e37287e5aa5614d9a963909c17953d066ea4df8a4e4e57e5b40fa096fb1646a',
      bytes: 452_298,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      '33f818fd52c2b10b2e1b2f77a72ee57443c96e176a43788de16c260649888843',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      'f6220b1af0857f11883e452ea30798bbd97ff73e978adb7c0c2bc7faba513b12',
    );
