...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: 'b31690c140ee51873d4f722bca508eacd006484e32417d50ca617c2451d4753b',
        acceptedBytes: 109_139,
      },
    ]);
  ...
  it('pins the final 817-item visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(109_139);
    expect(sha256(verdictBytes)).toBe(
      'b31690c140ee51873d4f722bca508eacd006484e32417d50ca617c2451d4753b',
    );
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: 'f70fd7e7a7ff7c53a73dc3991e8552c6c64f969ee0cee67e1126cf65589d5d3b',
      bytes: 454_295,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      '4534adf74bd8323c4e4ab3285a343483f502033831d7cbf0b93fb1e5983ca06e',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      '4975eb144bad974a0a58fd1fa223f4230115b975b2c88ef1892e9179dbd3e0cb',
    );
