...
describe('real catalog integration', () => {
  it('pins the fresh-character completion denominator of the live catalog', () => {
    ...
    // 222 deeds - 3 feats - 9 hidden = 210 visible to a fresh character.
    expect(view.summary.visibleTotal).toBe(210);
    expect(view.categories.reduce((n, c) => n + c.visible, 0)).toBe(213);
  });
