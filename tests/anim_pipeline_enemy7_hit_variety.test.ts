...
describe('ENEMY7 hit-reaction stagger (issue #2889 round 2)', () => {
  ...
  it('wires a matching animUrls entry onto both ENEMY7 consumers', () => {
    ...
    // kaykit()/skeletonClips(), BIPED14/YETI_BIPED14/TROLL_BIPED14, etc,
    // issue #2889) independently wire their own hit-variety donors in the
    // same batch and would otherwise inflate an unscoped count every time
    // one of them lands.
    const donorPattern = /goblin_hit_variety_anims\.glb|giant_hit_variety_anims\.glb/g;
