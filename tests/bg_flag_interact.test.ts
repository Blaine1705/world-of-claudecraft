...
describe('bgFlagGrabbableNearby', () => {
  ...
  it('is true just past BG_PICKUP_RADIUS, inside the small client reach margin', () => {
    // A predictive mirror of bgFlagAction's own reach check, not the
    // authority: online, the flag entity's mirrored position lags one
    // snapshot and the player's own position is locally predicted, so the
    // two can disagree by a hair right at the boundary. The margin makes
    // this gate a superset of the server's, never a subset: erring toward
    // attempting the grab (the server can still correctly refuse it) beats
    // silently routing an eligible press elsewhere.
    const enemyFlag = flagEntity(1, 10, 20);
