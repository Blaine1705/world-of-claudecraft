...
describe('deed_i18n English resolution', () => {
  ...
  it('manifests one row per name and desc plus one per title reward', () => {
    ...
    // 322 with the two Clue Scroll casket deeds, each with a name and desc;
    // the tenth carries the Treasure Hunter title.
    expect(manifest.filter((row) => row.field === 'name').length).toBe(322);
    ...
    // Mastery deeds. 303 with the two Clue Scroll casket deeds.
    expect(manifest.filter((row) => row.field === 'desc').length).toBe(303);
    // 676 rows: 322 names + 303 descs + 51 titles (the three faction Champion
    // titles Riftwarden, Dawnkeeper and Forgemaster join the 47, then the
    // Clue Scroll Treasure Hunter title).
    expect(manifest.length).toBe(676);
