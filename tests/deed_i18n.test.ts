...
describe('deed_i18n English resolution', () => {
  ...
  it('manifests one row per name and desc plus one per title reward', () => {
    ...
    expect(manifest.filter((row) => row.field === 'name').length).toBe(308);
    // 289 descs at the release/v0.43.0 merge: plus the eight world-quest deeds.
    expect(manifest.filter((row) => row.field === 'desc').length).toBe(289);
    // 644 rows at the release/v0.43.0 merge: 308 names + 289 descs + 47 titles.
    expect(manifest.length).toBe(644);
