...
function hoard(rarity: 'common' | 'rare' | 'epic' | 'legendary'): {
  ...
  sim.chat(`/dev hoard grask ${rarity}`, sim.player.id);
...
describe('the hoard reward chest', () => {
  it('stands on the room floor, not under it, in a room with a raised tier', () => {
    // A rare hoard's dais sits on the room's raised tier (a common one may be
    // flat; the tier is what this guards).
    const { sim, inst, boss } = hoard('rare');
...
describe('the dev hoard command', () => {
  it('digs the room each rarity really opens: only epic and legendary are open air', () => {
    expect(vaultSeedOpen(devHoardDestination('grask', 'common')!.seed)).toBe(false);
    expect(vaultSeedOpen(devHoardDestination('grask', 'rare')!.seed)).toBe(false);
    ...
    expect(devHoardDestination('grask', 'common')!.tier).toBe('C');
