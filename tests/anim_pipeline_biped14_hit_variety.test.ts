...
describe('BIPED14 hit-reaction stagger (issue #2889 round 2)', () => {
  ...
  it('wires a matching animUrls entry onto every BIPED14 consumer', () => {
    ...
    for (const [key, file] of consumers) {
      ...
      // mob_troll, mob_yeti, mob_murloc, mob_demon, and mob_demonalt each
      // wire their own `{ ...BIPED14, attack: [...] }` variant
      // (TROLL_BIPED14, YETI_BIPED14, MURLOC_BIPED14, DEMON_BIPED14, issue
      // #2889): each inherits BIPED14's hit array unchanged, so they still
      // qualify as BIPED14 consumers for HitReact_Heavy.
    ...
    // (e.g. ENEMY7's goblin/giant donors) whenever they land their own
    // hit-variety clips, which happens constantly in this repo.
    const occurrences = [
