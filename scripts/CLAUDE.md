...
| PR automation | `pr_screenshots.mjs` + `pr_shot_targets.mjs` produce the before/after PR screenshots the root workflow requires (captured locally and committed under `docs/screenshots`, never posted by CI); `prepare_ai_review.mjs` + `post_ai_review.mjs` + `redact_secrets.mjs` (+ `gh_sticky_comment.mjs`) power the CI review bot. See `docs/ai-pr-bot.md`. | dev / CI |
