...
| Group | Files | Needs |
|---|---|---|
| Gate / CI | `gate.mjs` (`npm run gate`): **full merge bar**, local mirror of the ci.yml PR-tier steps, sequential, stops at the first failure; release-tier (`I18N_RELEASE_TIER=1`) automatic on `release/**` branches; first checks node_modules is in sync with package-lock.json via `npm ls --depth=0 --json` (pure parse/format in `lib/npm_install_sync.mjs`; a drifted local install otherwise surfaces minutes later as a confusing tsc/build failure, never in CI since `npm ci` always resets it there), then hard-exits if the resolved `ffmpeg`/`ffprobe` binaries fail an execution probe (resolution: `sfx/ffmpeg_paths.mjs`, static packages with PATH fallback; the two preflights are pinned by `tests/dependency_sync_gate_preflight.test.ts` and `tests/sfx_gate_preflight.test.ts` respectively, with the pure npm-ls parse/format logic pinned separately in `tests/npm_install_sync.test.ts`). Workers: `lib/gate_workers.mjs` (CPU/2 + free-mem clamp; optional `GATE_WORKER_TIER` cap; `GATE_MAX_WORKERS` expert override). Keep its step list in sync with `.github/workflows/ci.yml`. Day-loop only: `gate_fast.mjs` (`npm run gate:fast`) is a subset (malware, biome changed, architecture+localization guards, `check:ts`, vitest `--changed`); pure plan in `lib/gate_fast_plan.mjs`. Measurement: `gate_profile.mjs`. | none (bundled FFmpeg) |
