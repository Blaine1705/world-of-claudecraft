...
await sleep(300);
await page.evaluate(() => (document.querySelector('#offline-select .mini-class[data-class="warrior"]') || document.querySelector('.class-card[data-class="warrior"]'))?.click());
await sleep(150);
...
await page.evaluate(() => document.querySelector('.tal-tab[data-tab="rows"]')?.dispatchEvent(new MouseEvent('click', { bubbles: true })));
await sleep(400);
const rows = await page.evaluate(() => ({
  ...
  firstNames: [...document.querySelectorAll('.tal-row:first-child .tal-row-opt b')].map((b) => b.textContent),
}));
...
await page.evaluate(() => document.querySelector('.tal-row-opt[data-opt="war_row_crushing_charge"]')?.click());
await sleep(500);
const after = await page.evaluate(() => ({
  simPick: window.__game.sim.rowPicks[0],
  pressed: document.querySelector('.tal-row-opt[data-opt="war_row_crushing_charge"]')?.getAttribute('aria-pressed'),
  badge: document.querySelector('.tal-tab[data-tab="rows"] .tt-pts')?.textContent,
  chargeAdds: window.__game.sim.entities ? (window.__game.sim.talents, (window.__game.sim.player, null)) : null,
}));
...
await page.evaluate(() => { window.__game.sim.setPlayerLevel(8); window.__game.hud.toggleTalents(); window.__game.hud.toggleTalents(); });
await sleep(300);
await page.evaluate(() => document.querySelector('.tal-tab[data-tab="rows"]')?.dispatchEvent(new MouseEvent('click', { bubbles: true })));
await sleep(300);
