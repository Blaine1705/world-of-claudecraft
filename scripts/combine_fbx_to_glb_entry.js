...
function stripArmature(clip) {
  clip.tracks = clip.tracks.filter((tr) => !/(^|[^A-Za-z])Armature\.(quaternion|position|scale)$/.test(tr.name));
}
function loadImage(dataUrl) {
  return new Promise((res, rej) => {
    const img = new Image(); img.onload = () => res(img);
    img.onerror = () => rej(new Error('image decode failed')); img.src = dataUrl;
  });
}
...
window.combine = async (files, opts) => {
  ...
  const parsed = files.map((f) => ({ name: f.name, group: new FBXLoader().parse(b64ToAB(f.b64), '') }));
  const baseEntry = parsed.find((p) => {
    let has = false; p.group.traverse((o) => { if (o.isSkinnedMesh || (o.isMesh && o.skeleton)) has = true; });
    return has;
  }) || parsed[0];
  ...
  const addFrom = (entry) => {
    ...
    take.forEach((clip, i) => {
      const meaningful = clip.name && !/^mixamo\.com/i.test(clip.name) && !/^Armature\|/i.test(clip.name);
  ...
  const mats = new Set();
  base.traverse((o) => { if (o.isMesh) (Array.isArray(o.material) ? o.material : [o.material]).forEach((m) => m && mats.add(m)); });
  if (opts.externalTexB64) {
    ...
    for (const m of mats) {
      ...
      m.map = tex; if (m.color) m.color.set(0xffffff);
    ...
    while (mapped.some((m) => !(m.map.image && m.map.image.width)) && performance.now() - t0 < 10000) {
    ...
    for (const m of mats) {
      ...
      if (src && src.image && src.image.width) {
        const img = src.image; const cv = document.createElement('canvas');
        cv.width = img.width; cv.height = img.height; cv.getContext('2d').drawImage(img, 0, 0);
        const tex = new THREE.CanvasTexture(cv);
        tex.flipY = src.flipY; tex.colorSpace = src.colorSpace || THREE.SRGBColorSpace;
        tex.wrapS = src.wrapS; tex.wrapT = src.wrapT; m.map = tex; kept++;
      } else m.map = null;
      m.emissiveMap = m.normalMap = m.roughnessMap = m.metalnessMap = m.aoMap = null;
    }
  ...
  if (opts.targetHeight) {
    ...
    base.scale.multiplyScalar(s); base.updateMatrixWorld(true); dbg.scale = +s.toFixed(5);
  }
  ...
  if (opts.restClip != null && opts.restT != null) {
    ...
    if (idle) {
      ...
      mixer.clipAction(idle).play(); mixer.update(opts.restT); base.updateMatrixWorld(true);
      dbg.posedAt = `${idle.name}@${opts.restT}`;
    }
  }

  const glb = await new Promise((res, rej) =>
    new GLTFExporter().parse(base, res, rej, { binary: true, animations: clips, onlyVisible: false }));
