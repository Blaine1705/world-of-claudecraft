...
**Technical and log data.** When you connect to the Service we automatically receive technical information, including your IP address, device and browser type, operating system, language and display settings, session identifiers, session tokens (which expire after 7 days), connection timestamps, error and performance logs, and, for the desktop app, hardware and power details of your computer (such as memory size, memory use, power mode, and graphics scheduling settings). We use your IP address to rate-limit sign-in attempts and to detect and prevent abuse.
