...
|---|---|---|
| `system` | `os{caption,version,build,architecture,uptimeHours}`, `cpu{name,cores,threads,maxClockMHz}`, `machine{manufacturer,model}`, `isLaptop`, `laptopHints{...}`, `systemDrive{sizeGB,freeGB}` | `isLaptop` = 2 out of 3 vote between chassis type, PCSystemType and battery presence; the raw hints are provided |
...
| `browsers` | `defaultBrowserProgId`, `browsers[]{name,exe,installed,version,running,hardwareAccelerationDisabledByUser,note}` | only the hardware-acceleration key is read from the browser profile. `false` does not prove the GPU is used (launch flags, driver blocklist): the ground truth is `sampling.gpuApps` |
